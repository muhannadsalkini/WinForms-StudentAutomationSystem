...
namespace WindowsFormsApp_StudentAutomayionSystem.Student
{
    public partial class CourseSchedule : Form
    {
        ...
        private void Course_Schedule_Load(object sender, EventArgs e)
        {
            ...
            string sqlstatment = "SELECT day, hall_no, startTime, endTime, Lectures.name as lecture, Teachers.name as teacher," +
                " prefix FROM StudentLectures, Teachers, Lectures WHERE lecture_id = Lectures.Id AND Teachers.Id = Lectures.teacher_id AND student_id = '" +
                studentId + "'";
